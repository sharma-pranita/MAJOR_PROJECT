...
async function connectDB() {
  ...
  try {
    ...
    console.log(`Connected successfully to MongoDB at ${config.mongoUrl}`);
  } catch (err) {
    console.warn(`Failed to connect to configured MONGO_URL: ${err.message}`);
    console.warn('Falling back to local MongoDB at mongodb://localhost:27017...');
    client = new MongoClient('mongodb://localhost:27017');
    await client.connect();
    console.log('Connected successfully to local MongoDB at mongodb://localhost:27017');
  }
...
async function getUserFiles(userId) {
  const database = getDB();
  return await database.collection('files')
    .find({ user_id: userId })
    .project({ _id: 0 })
    .toArray();
}
...
async function getFileById(fileId, userId) {
  const database = getDB();
  return await database.collection('files').findOne({ id: fileId, user_id: userId });
}
...
async function deleteFile(fileId, userId) {
  const database = getDB();
  const result = await database.collection('files').deleteOne({ id: fileId, user_id: userId });
